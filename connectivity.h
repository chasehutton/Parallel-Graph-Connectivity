...
auto cluster(const parlay::sequence<parlay::sequence<int>>& G, float beta, bool permute) {
	...
	auto buckets = parlay::group_by_index(parlay::delayed::tabulate(n, [&] (int i) {
	...
	for (int i = 0; i < buckets.size(); i++) {
		...
		auto pairs = parlay::delayed::flatten(parlay::delayed::map(frontier, [&] (int v) {
			return parlay::delayed::map(G[v], [=] (int u) {
				return std::pair(v,u);
			});
		}));
		...
		// the vertices id
		frontier = parlay::delayed::to_sequence(parlay::delayed::map_maybe(pairs, [&] (auto p) {
			auto [v,u] = p;
...
auto contract(const parlay::sequence<parlay::sequence<int>>& G, const parlay::sequence<int>& clusters, int num_clusters, bool* stop) {
	...
	auto potential_edges = parlay::delayed::flatten(parlay::delayed::tabulate(G.size(), [&](int u) {
        return parlay::delayed::map_maybe(G[u], [&](int v) {
	...
	auto edges = parlay::remove_duplicates(parlay::delayed::to_sequence(potential_edges));
